...
mringf_infoの書き込み頻度削減による書き込み量の違いを測定します。

## 仕様

- **5000個のdatファイル**: `/opt/emmc_test/data/00000.dat` ~ `04999.dat` (各64 bytes)
- **mringf_infoファイル**: `/opt/emmc_test/data/mringf_info` (440KB)
- **スレッドプール方式**: 指定した数のスレッドで5000ファイルを分担処理
- **テストモード**:
  - **モード0 (修正前)**: 各datファイル更新後に毎回mringf_infoを更新
  - **モード1 (修正後)**: 30秒周期で1回だけmringf_infoを更新
...
|-------|-----------|----------------|--------------|------------|
| 0 (修正前) | ~90 MB | ~9,000回 | ~3,900 MB | **~4,000 MB** |
| 1 (修正後) | ~90 MB | 2回 | ~0.9 MB | **~91 MB** |
